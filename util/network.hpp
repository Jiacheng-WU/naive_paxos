...
using Handler = std::function<void(std::unique_ptr<Message> m_p,
                                   std::unique_ptr<boost::asio::ip::udp::endpoint> endpoint, asio_handler_paras paras)>;
inline Handler do_nothing_handler = [](std::unique_ptr<Message> m_p,
...
class Connection {
  public:
    ...
    void
    resend_if_do_send_failed(std::unique_ptr<Message> m_p, std::unique_ptr<boost::asio::ip::udp::endpoint> endpoint,
                             Handler handler) {
        struct inner_retry {
            std::uint32_t times;
            Connection *connect;
            ...
            void operator()(std::unique_ptr<Message> m_p, std::unique_ptr<boost::asio::ip::udp::endpoint> endpoint,
                            asio_handler_paras paras) {
                if (this->times > 0) {
    ...
    void
    do_send(std::unique_ptr<Message> m_p, std::unique_ptr<boost::asio::ip::udp::endpoint> endpoint, Handler handler);
